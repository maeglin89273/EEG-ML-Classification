...
try:
  ...
  from nupic.algorithms import anomaly_likelihood as alh
except ImportError:
  pass
...
class NuPICPlotOutput(NuPICOutput):
  ...
  def __init__(self, *args, **kwargs):
    ...
    if self.show_anomaly_score:
      ...
      self.anomaly_likelihood = deque([0.0] * WINDOW, maxlen=360)
      self.anomaly_likelihood_algorithm = alh.AnomalyLikelihood(0,1);
    ...
    if self.show_anomaly_score:
      ...
      self.anomaly_likelihood_line, = plt.plot(range(WINDOW), self.anomaly_score, 'b-')
    ...
    if self.show_anomaly_score:
      self.anomaly_score_line.axes.set_ylim(0, 1)
      self.anomaly_likelihood_line.axes.set_ylim(0, 1)
  ...
  def write(self, index, key, value, prediction_result, prediction_step=1):
    ...
    if inference is not None:
      ...
      if self.show_anomaly_score:
        ...
        likelihood = self.anomaly_likelihood_algorithm.anomalyProbability(value, anomaly_score, index)
        self.anomaly_likelihood.append(likelihood)
